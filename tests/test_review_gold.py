...
def test_future_match_does_not_change_features():
    ...
    past = pd.DataFrame([
        ...
        {"date": "2026-01-03", "home_team": "C", "away_team": "D",
    ...
    future = pd.DataFrame([
        ...
        {"date": "2026-01-03", "home_team": "C", "away_team": "D",
         "home_goals": 0, "away_goals": 1},
        {"date": "2026-01-10", "home_team": "A", "away_team": "C",
         "home_goals": 4, "away_goals": 0},   # 未来赛果
    ])
    fut = with_elo_features(future)
    for col in ("home_elo", "away_elo", "elo_diff", "attack_elo_diff"):
        assert np.allclose(base[col].to_numpy(), fut[col].iloc[:len(base)].to_numpy()),             f"{col} 被未来数据污染"
...
def test_goal_weights_normalize():
    ...
    assert abs(out1["fused_lams"][0] - (0.6*1.5 + 0.2*1.5 + 0.05*1.5 + 0.15*1.8)) < 1e-12
...
def test_prediction_determinism():
    ...
    from app.prediction.goal_engine import compute_members
    kwargs = dict(lam_h=1.5, lam_a=1.2, lam_eh=1.8, lam_ea=1.0,
                  tau=0.05, phi=30.0, weights=_base_weights(0.1))
...
def test_learn_weights_removes_missing_member():
    ...
    samples = [
        {"hgbr": [0.5, 0.3, 0.2], "dc": [0.4, 0.3, 0.3],
         "elo": [0.6, 0.2, 0.2], "actual": 0},
        {"hgbr": [0.2, 0.3, 0.5], "dc": [0.3, 0.3, 0.4],
         "elo": [0.2, 0.3, 0.5], "actual": 2},
        {"hgbr": [0.3, 0.5, 0.2], "dc": [0.3, 0.4, 0.3],
         "elo": [0.3, 0.4, 0.3], "actual": 1},
    ]
