...
def main():
    ...
    ap.add_argument("--league", default=None, help="enrich 联赛(默认全部 5 大联赛)")
    ap.add_argument("--season", default="2024", help="enrich 赛季起始年(默认 2024)")
...
def _run_zafronix(league: str | None, year: str | None):
    ...
    from app.data.sources.zafronix import TOURNAMENT_PATHS, ingest_year
    from app.data.sources.zafronix import available as zx_available
    ...
    with session_scope():
        for lt in leagues:
            ...
            rows = (
                ...
                .filter(
                    (Match.home_shots.is_(None)) | (Match.home_possession.is_(None))
                )
            ...
            try:
                ...
                print(
                    f"    → 更新 {res['updated']} | 未匹配 {res['unmatched']} | "
    ...
    with session_scope():
        for lt in leagues:
            ...
            try:
                ...
                print(
                    f"    → 更新 {res['updated']} | 未匹配 {res['unmatched']} | 错误 {res['errors']} | 源 {res['fetched']} 行"
                )
            except Exception as e:
                print(f"    ✗ {type(e).__name__}: {e}")
