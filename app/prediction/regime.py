"""Regime Detector + Score Matrix 级调整(

:Prior Blend 只改 1X2 而 score matrix 不变 → 两套输出统计不一致。
...
:Regime Detector —— 检测平局率/进球率/主场优势漂移,输出 regime
标签;α 动态化:稳定赛季 α≈0.85,剧烈 shift α≈0.55(
"""
...
def detect(
 ...
 A70A601 :实现与注释统一 —— 取该队**最近 5 场**(ms 已按日期
