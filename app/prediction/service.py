...
def predict_match(league_type: LeagueType, home_team: str, away_team: str,
    ...
    league = __import__("app.api.db", fromlist=["League"]).League.query.filter_by(
        league_type=league_type.value).first()
    if league is None:
    ...
    snapshot_service.save(
        league, home_team, away_team, match_dt, match_date, result,
        _internal.get("home_lambda"), _internal.get("away_lambda"),
        _internal.get("att_diff"), _internal.get("hist_df"),
        _internal.get("model"), _internal.get("_pred_df"),
        _internal.get("_m"), hist_max_id, hist_max_updated,
        __import__("app.models.registry", fromlist=["_model_path"])._model_path,
        models_dir, _internal.get("cal_info"),
        _feat=_internal.get("_feat"), _score_matrix=_internal.get("fused_matrix"),
