...
_CACHE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "raw", "injuries")
...
class InjuriesCollector:
    ...
    def _get_json(self, url: str, headers: dict | None = None, timeout: int = 20) -> dict:
    ...
    def dedupe(records: list[dict]) -> list[dict]:
        ...
        out = []
        for r in records:
            key = ((r.get("player") or {}).get("id"), (r.get("fixture") or {}).get("id"))
            if key in seen:
                continue
            seen.add(key)
            out.append(r)
    ...
    # ---------------- 采集(带缓存) ----------------
    def fetch_by_date(self, day: str | None = None, use_cache: bool = True) -> list[dict]:
    ...
    def filter_by_team(self, records: list[dict], team_name: str) -> list[dict]:
        ...
        for r in records:
            ...
            if not t:
                continue
            if target and (self._norm(t) == target or self._norm(t) in target or target in self._norm(t)):
                out.append(r)
        return out
    ...
    def build_brief(self, home: str, away: str, match_date: str | None = None) -> dict:
        ...
        day = (match_date or date.today().isoformat())[:10]
        ...
        lines.append(f"- api-football 伤停: 主队 {len(home_inj)} 人, 客队 {len(away_inj)} 人")
        for tag, rs in (("主队", home_inj), ("客队", away_inj)):
            for it in _fmt(rs):
                lines.append(f"  • [{it['type']}] {it['player']} ({it['reason']}) — {tag}")
        return {
