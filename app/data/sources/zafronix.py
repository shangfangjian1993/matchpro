...
def _key() -> str:
    ...
    def fetch_matches(
        ...
        if path is None:
            raise ValueError(
                f"Zafronix 未支持: {league_type_value}(可用 {list(TOURNAMENT_PATHS)})"
            )
    ...
    def to_normalized(raw: dict, league_type_value: str, year: int) -> NormalizedMatch:
        ...
        season_label = (
            f"{year}-{year + 1}"
            if raw.get("season") is None
            else str(raw.get("season"))
        )
        return NormalizedMatch(
            ...
            home_goals=int(raw["homeScore"])
            if raw.get("homeScore") is not None
            else None,
            away_goals=int(raw["awayScore"])
    ...
    def ingest_year(league_type_value: str, year: int, verbose: bool = True) -> dict:
        ...
        while True:
            batch = fetch_matches(
                league_type_value, year=year, limit=100, cursor=cursor
            )
            rows.extend(batch)
            if len(batch) < 100:
                break
