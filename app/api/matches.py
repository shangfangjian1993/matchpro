...
def _resolve_league(data: dict):
 ...
 if league is None:
 raise HTTPException(
 404, f"数据库中还没有 {league_type.value} 的联赛,请先创建联赛"
 )
 return league
...
def _validate_match(league, data: dict, index: int | None = None):
 ...
 match_date = (
 _parse_date(data.get("match_date")) if data.get("match_date") else utcnow()
 )
 m = Match(
 league_id=league.id,
 home_team=home,
 ...
 try:
 setattr(
 m, col, float(data[col]) if col in FLOAT_FIELDS else int(data[col])
 )
 except (TypeError, ValueError):
...
def create_match(body: MatchCreate, user=Depends(get_current_user)):
 ...
 league_id=league.id, home_team=m.home_team, away_team=m.away_team
 )
 .filter(Match.match_date == m.match_date)
 .first()
 )
 if dup:
 return {
 "message": "该场比赛已存在,未重复录入",
 "match": dup.to_dict(),
 "duplicate": True,
 }
 db.session.add(m)
...
def batch_create_matches(body: MatchBatchReq, user=Depends(get_current_user)):
 ...
 400,
 {
 "message": f"{len(errors)} 条数据校验失败,未写入任何数据",
 "errors": errors[:20],
 },
 )
 imported, skipped = [], 0
 ...
 db.session.commit()
 return {
 "message": f"批量录入完成:新增 {len(imported)} 场,跳过重复 {skipped} 场",
