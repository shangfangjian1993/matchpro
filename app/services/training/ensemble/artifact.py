...
class ProductionArtifact:
 ...
 : 使用 typed schema (GoalLambdaWeights/ScoreDistributionWeights/OutcomeWeights)。
 """
 ...
 """序列化为 dict(: 全精度,无 round/renormalize)。"""
 return {
 "league": self.league,
...
def create_production_artifact(
 ...
 : training_cutoff 由调用方传入(真实数据截止时间)。
 : oof_segments 由调用方传入(真实 K_SEG)。
 """
