"""HGBR 超参搜索(评审 .1):在英超 holdout 上按 Log-Loss/RPS 选优。
...
def main():
 ...
 model = LeagueModelFactory.create_league_model(
 LeagueType.PREMIER_LEAGUE, cfg
 )
 try:
 ...
 print(
 f" [{i}/{len(CANDIDATES)}] lr={params['learning_rate']} "
 f"depth={params['max_depth']} → ll={pm.get('log_loss', 'N/A')} "
 f"brier={pm.get('brier', 'N/A')} rps={pm.get('rps', 'N/A')}",
 flush=True,
 )
 ...
 r
 for r in valid
 if r["params"]["learning_rate"] == 0.06 and r["params"]["max_depth"] == 6
 ]
 if base:
 print(
 "基线(lr=0.06 d=6):",
 {k: base[0][k] for k in ("log_loss", "brier", "rps")},
 )
 ...
 with open(os.path.join(_exp_dir, "best.json"), "w", encoding="utf-8") as f:
