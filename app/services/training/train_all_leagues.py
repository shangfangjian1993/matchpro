    ...
    with session_scope():
        ...
        for name in league_names:
            ...
            try:
                ...
                if record is None:
                    record = ModelRecord(name=f"{lt.value}模型", league_type=lt.value,
                ...
                record.version = str(metrics.get("model_version", record.version or "1.0.0"))
                ...
                print(f"  OK v{metrics.get('model_version')} acc={metrics.get('exact_accuracy'):.4f} "
                      f"poisson={metrics.get('poisson_loss'):.4f} shape={metrics.get('training_data_shape')} ({secs:.0f}s)", flush=True)
