...
def _run_bzzoiro(fn_name: str, league: str) -> dict:
    ...
    r = fn(league)
    return {"ok": True, "detail": {k: r[k] for k in ("fetched", "inserted", "updated") if k in r} or r}


def _run_understat(season) -> dict:
    from app.data.pipeline import run_xg
    r = run_xg([season], ["E0", "SP1", "D1", "I1", "F1"])
    return {"ok": not r.get("errors"), "detail": {"updated": r.get("updated"), "errors": len(r.get("errors", []))}}


def _run_fdco(season) -> dict:
    from app.data.pipeline import run_history
    r = run_history([season], ["E0", "SP1", "D1", "I1", "F1"])
    return {"ok": not r.get("errors"), "detail": {"inserted": r.get("inserted"), "updated": r.get("updated"),
...
_TYPE_RUNNERS = {
    "results": {"bzzoiro_events": lambda: _run_bzzoiro("import_recent", "premier_league"),
...
def _run_bzzoiro_incr(fn_name: str) -> dict:
    ...
    from app.data.sources.bzzoiro import LEAGUE_IDS, _get, _event_match_index
    if not bzzoiro.available():
        return {"ok": False, "detail": "缺 BZZOIRO_KEY"}
    total_done = 0
    for league in _LEAGUES_EURO:
        ...
        if lid is None:
            continue
        idx = _event_match_index(league)
        # 窗口截止
        d0 = _get("/events/", {"league_id": lid, "status": "finished", "limit": 1})
        latest = dt.datetime.fromisoformat(d0["results"][0]["event_date"].replace("Z", "+00:00"))
        last_season = latest.year if latest.month >= 8 else latest.year - 1
        cutoff = dt.datetime(last_season - STATS_SEASONS + 1, 8, 1, tzinfo=dt.timezone.utc)
        total = d0.get("count", 0)
        ofs, done = 0, 0
        while ofs < total:
            batch = bzzoiro.fetch_events(league_id=lid, status="finished", limit=200, offset=ofs)
            if not batch:
                break
            batch = [e for e in batch if dt.datetime.fromisoformat(
                e["event_date"].replace("Z", "+00:00")) >= cutoff]
            if batch:
                fn = getattr(bzzoiro, fn_name)
...
def run_type(dtype: str) -> dict:
    ...
    for source in (primary, fallback):
        ...
        logger.warning("源 %s 未达标(fallback→%s): %s", source, fallback, res.get("detail"))
...
def main():
    ...
    ap.add_argument("--type", dest="dtype", choices=list(
        __import__("app.data.sources_registry", fromlist=["PRIMARY_FALLBACK"]).PRIMARY_FALLBACK))
    args = ap.parse_args()
    import json
    from app.api.db import init_db
    init_db()
    report = run_frequency(args.freq) if args.freq else {args.dtype: run_type(args.dtype)}
