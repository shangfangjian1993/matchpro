...
def main():
    ...
    with session_scope():
        ...
        latest_fv = {
            lt: max(items, key=lambda x: x[0])[1] for lt, items in fv_by_lt.items()
        }
        best = {}
        for e in rows:
            lt = getattr(e, "league_type", None) or ""
        ...
        meta = {
            "selected_by": "selection-holdout(poisson0.40+ll0.25+brier0.20+rps0.15)",
        ...
        for lt, v in sorted(best.items()):
            print(
                f"   {lt}: v{v['version']} (score {v['score']:.4f} | "
