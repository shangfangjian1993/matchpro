...
def fuse_probs(member_probs_list: dict[str, tuple[float, float, float]],
               weights: dict | None = None) -> tuple[float, float, float]:
...
def fuse_goal_outcome(goal_probs: tuple[float, float, float],
    ...
    total = w_goal + w_gbm
    return tuple(round(w_goal / total * g + w_gbm / total * b, 6)
