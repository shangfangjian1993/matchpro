...
def score_outputs(matrix: np.ndarray) -> dict:
    ...
    flat = [(i, j, float(m[i, j])) for i in range(10) for j in range(10)]
    flat.sort(key=lambda t: -t[2])
    top5 = [{"home": i, "away": j, "probability": round(p, 5)} for i, j, p in flat[:5]]
    over = sum(m[i, j] for i in range(10) for j in range(10) if i + j > 2)
    btts = sum(m[i, j] for i in range(10) for j in range(10) if i > 0 and j > 0)
    xg_home = sum(i * m[i, j] for i in range(10) for j in range(10))
    xg_away = sum(j * m[i, j] for i in range(10) for j in range(10))
    return {
        "top_scores": top5,
