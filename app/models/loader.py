"""模型加载器(审查 §14/§39 拆分:自 data_adapter 迁移)。
...
def _load_model(league_type: LeagueType, models_dir: str, version: str = "latest"):
    ...
    path = _model_path(league_type, models_dir, None if version == "latest" else version)
    if not os.path.exists(path):
        if version not in ("latest", ""):
            logger.warning("%s 版本 %s 不存在(可能被 prune),回退到文件系统最新版本",
                           league_type.value, version)
            version = "latest"
