...
def backfill_snapshot(snapshot, match) -> bool:
 ...
 pred = max(
 probs, key=lambda k: probs[k] if k in ("home_win", "draw", "away_win") else -1
 )
 snapshot.is_correct = (
 (pred == "home" and gh > ga)
 or (pred == "draw" and gh == ga)
 or (pred == "away" and gh < ga)
 )
 p_actual = probs.get(
 {"home": "home_win", "draw": "draw", "away": "away_win"}[actual], 0.0
 )
 ...
 "goal_mae": round(
 (
 abs(ev.get("lambda", [0, 0])[0] - gh)
 + abs(ev.get("lambda", [0, 0])[1] - ga)
 )
 / 2,
 4,
 ),
...
def replay_all(force: bool = False) -> dict:
 ...
 PredictionSnapshot.actual_home_goals.is_(None)
 ).all()
 if not force
...
def summarize() -> dict:
 ...
 done = PredictionSnapshot.query.filter(
